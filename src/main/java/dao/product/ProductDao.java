...
public class ProductDao implements IProductDao {
    ...
    public List<Product> getNewProduct(int lenght) {
        List<Product> products = new ArrayList<>();
        try {
            PreparedStatement statement = statement = connection.prepareStatement("select * from product order by id desc limit ?;");
            statement.setInt(1,lenght);
            ResultSet rs = statement.executeQuery();
            while(rs.next()) {
                int id = rs.getInt("id");
                Product product = getProductInfor(id,rs);
    ...
    public List<Product> selectByCategory(int CategoryId,int limit) {
        List<Product> products = new ArrayList<>();
        try {
            PreparedStatement statement = statement = connection.prepareStatement("select * from product where category_id = ? limit ?;");
